...
function WebGLMorphtargets(gl) {
  ...
  function update(object, geometry, material, program) {
    ...
    const morphTargets = material.morphTargets &&
      geometry.morphAttributes.position;
    const morphNormals = material.morphNormals &&
    ...
    for (let i = 0; i < 8; i++) {
      ...
      if (index !== Number.MAX_SAFE_INTEGER && value) {
        ...
        if (
          morphTargets && geometry.getAttribute("morphTarget" + i) !== undefined
        ) {
          geometry.deleteAttribute("morphTarget" + i);
        }

        if (
          morphNormals && geometry.getAttribute("morphNormal" + i) !== undefined
        ) {
