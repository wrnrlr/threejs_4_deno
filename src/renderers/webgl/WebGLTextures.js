...
function WebGLTextures(
      ...
      if (
        ...
        console.warn(
          "THREE.WebGLRenderer: Texture has been resized from (" + image.width +
            "x" + image.height + ") to (" + width + "x" + height + ").",
        );
  ...
  function generateMipmap(target, texture, width, height) {
    ...
    textureProperties.__maxMipLevel = Math.log(Math.max(width, height)) *
      Math.LOG2E;
  }
  ...
  function getInternalFormat(internalFormatName, glFormat, glType) {
    ...
    if (
      internalFormat === _gl.R16F || internalFormat === _gl.R32F ||
  ...
  function filterFallback(f) {
    if (
      f === NearestFilter || f === NearestMipmapNearestFilter ||
    ...
    if (textureUnit >= maxTextures) {
      console.warn(
        "THREE.WebGLTextures: Trying to use " + textureUnit +
          " texture units while this GPU supports only " + maxTextures,
      );
    }
  ...
  function setTexture2D(texture, slot) {
    ...
    if (
      texture.version > 0 && textureProperties.__version !== texture.version
    ) {
  ...
  function setTextureParameters(textureType, texture, supportsMips) {
    if (supportsMips) {
      ...
      if (
        textureType === _gl.TEXTURE_3D || textureType === _gl.TEXTURE_2D_ARRAY
      ) {
        _gl.texParameteri(
          textureType,
          _gl.TEXTURE_WRAP_R,
  ...
  function uploadTexture(textureProperties, texture, slot) {
    ...
    const needsPowerOfTwo = textureNeedsPowerOfTwo(texture) &&
    ...
    if (texture.isDepthTexture) {
      ...
      if (
        ...
        if (
          texture.type !== UnsignedShortType && texture.type !== UnsignedIntType
        ) {
          console.warn(
    ...
    if (renderTarget.depthTexture) {
      ...
      if (isCube) {
        ...
        for (let i = 0; i < 6; i++) {
          ...
          renderTargetProperties.__webglDepthbuffer[i] = _gl
            .createRenderbuffer();
  ...
  function setupRenderTarget(renderTarget) {
    ...
    if (
      isWebGL2 && renderTarget.texture.format === RGBFormat &&
    ...
    if (isCube) {
      ...
      if (isMultisample) {
        if (isWebGL2) {
          renderTargetProperties.__webglMultisampledFramebuffer = _gl
            .createFramebuffer();
          renderTargetProperties.__webglColorRenderbuffer = _gl
            .createRenderbuffer();
          ...
          if (renderTarget.depthBuffer) {
            renderTargetProperties.__webglDepthRenderbuffer = _gl
              .createRenderbuffer();
            setupRenderBufferStorage(
