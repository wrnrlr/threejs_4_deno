...
var Fire = function (geometry, options) {
  ...
  this.expansion = (options.expansion === undefined)
    ? -0.25
    : options.expansion;
  ...
  this.massConservation = (options.massConservation === undefined)
    ? false
    : options.massConservation;
    ...
    for (var y = startY; y < endY; y++) {
      for (var x = startX; x < endX; x++) {
        ...
        if (diffX * diffX + diffY * diffY < radius * radius) {
          ...
          if (density != null) {
            this.sourceData[stride] = Math.min(Math.max(density, 0.0), 1.0) *
              255;
          }
  ...
  this.configShaders = function (dt) {
    ...
    this.diffuseMaterial.uniforms["viscosity"].value = dt * 0.05 *
      this.viscosity;
    ...
    this.driftMaterial.uniforms["airSpeed"].value = dt * this.airSpeed * 0.001 *
      textureHeight;
