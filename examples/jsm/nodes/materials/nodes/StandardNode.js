  ...
  if (builder.isShader("vertex")) {
    ...
    if (this.environment) {
      ...
      this.environment.analyze(
        builder,
        { cache: "radiance", context: contextEnvironment, slot: "radiance" },
      );
    ...
    var color = this.color.flow(
      builder,
      "c",
      { slot: "color", context: contextGammaOnly },
    );
    ...
    if (this.environment) {
      environment = {
        radiance: this.environment.flow(
          builder,
          "c",
          { cache: "radiance", context: contextEnvironment, slot: "radiance" },
        ),
      ...
      if (builder.requires.irradiance) {
        environment.irradiance = this.environment.flow(
          builder,
          "c",
          {
            cache: "irradiance",
    ...
    var clearcoatEnv = useClearcoat && environment
      ? this.environment.flow(
        builder,
        "c",
        {
          cache: "clearcoat",
    ...
    if (clearcoatRoughness) {
      output.push(
        ...
        "material.clearcoatRoughness = max( " + clearcoatRoughness.result +
          ", 0.0525 );",
    ...
    if (reflectivity) {
      output.push(
        ...
        "material.specularColor = mix( vec3( MAXIMUM_SPECULAR_COEFFICIENT * pow2( " +
          reflectivity.result + " ) ), diffuseColor, metalnessFactor );",
      );
    } else {
      output.push(
    ...
    if (ao) {
      output.push(
        ...
        "reflectedLight.indirectSpecular *= computeSpecularOcclusion( dotNV, " +
          ao.result + ", material.specularRoughness );",
      );
    }
