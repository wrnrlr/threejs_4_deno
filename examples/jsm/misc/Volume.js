...
Volume.prototype = {
  ...
  extractPerpendicularPlane: function (axis, RASIndex) {
    ...
    sliceAccess = function (i, j) {
      ...
      var si = (iDirection === axisInIJK)
        ? IJKIndex
        : (iDirection.argVar === "i" ? i : j);
      var sj = (jDirection === axisInIJK)
        ? IJKIndex
        : (jDirection.argVar === "i" ? i : j);
      var sk = (kDirection === axisInIJK)
        ? IJKIndex
        : (kDirection.argVar === "i" ? i : j);

      // invert indices if necessary

      var accessI = (iDirection.dot(base[0]) > 0)
        ? si
        : (volume.xLength - 1) - si;
      var accessJ = (jDirection.dot(base[1]) > 0)
        ? sj
        : (volume.yLength - 1) - sj;
      var accessK = (kDirection.dot(base[2]) > 0)
        ? sk
        : (volume.zLength - 1) - sk;
