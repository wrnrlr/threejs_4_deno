  ...
  parse: function (object, onDone, options) {
    ...
    function format(urdf) {
      ...
      return urdf
        ...
        .map((tag) => {
          if (
            !HAS_TEXT.test(tag) && !IS_SELF_CLOSING.test(tag) &&
            IS_END_TAG.test(tag)
          ) {
            tagnum--;
          }
    ...
    function getAttribute(attr, name, params, type) {
      ...
      var res = `<source id="${name}">` +
        `<float_array id="${name}-array" count="${array.length}">` +
        array.join(" ") +
        "</float_array>" +
        "<technique_common>" +
        `<accessor source="#${name}-array" count="${
          Math.floor(array.length / attr.itemSize)
    ...
    function processGeometry(g) {
      ...
      if (!info) {
        ...
        gnode +=
          `<vertices id="${vertName}"><input semantic="POSITION" source="#${posName}" /></vertices>`;
        ...
        // serialize normals
        var triangleInputs =
          `<input semantic="VERTEX" source="#${vertName}" offset="0" />`;
        if ("normal" in bufferGeometry.attributes) {
          ...
          triangleInputs +=
            `<input semantic="NORMAL" source="#${normName}" offset="0" />`;
        }
        ...
        if ("uv" in bufferGeometry.attributes) {
          ...
          triangleInputs +=
            `<input semantic="TEXCOORD" source="#${uvName}" offset="0" set="0" />`;
        }
        ...
        if ("uv2" in bufferGeometry.attributes) {
          ...
          triangleInputs +=
            `<input semantic="TEXCOORD" source="#${uvName}" offset="0" set="1" />`;
        }
        ...
        if ("color" in bufferGeometry.attributes) {
          ...
          triangleInputs +=
            `<input semantic="COLOR" source="#${colName}" offset="0" />`;
        }
        ...
        for (var i = 0, l = groups.length; i < l; i++) {
          ...
          gnode +=
            `<triangles material="MESH_MATERIAL_${group.materialIndex}" count="${polycount}">`;
    ...
    function processTexture(tex) {
      ...
      if (texid == null) {
        ...
        if (version === "1.5.0") {
          imageNode +=
            `<init_from><ref>${options.textureDirectory}${name}.${ext}</ref></init_from>`;
        } else {
          // version image node 1.4.1
          imageNode +=
            `<init_from>${options.textureDirectory}${name}.${ext}</init_from>`;
        }
    ...
    function processMaterial(m) {
      ...
      if (matid == null) {
        ...
        var effectnode = `<effect id="${matid}-effect">` +
          "<profile_COMMON>" +
          (
            m.map
        ...
        var materialNode =
          `<material id="${matid}"${materialName}><instance_effect url="#${matid}-effect" /></material>`;
    ...
    function processObject(o) {
      ...
      if (o instanceof Mesh && o.geometry != null) {
        ...
        matids = matidsArray.fill().map((v, i) =>
          processMaterial(materials[i % materials.length])
        );

        node += `<instance_geometry url="#${meshid}">` +
          (
            matids != null
    ...
    var specLink = version === "1.4.1"
      ? "http://www.collada.org/2005/11/COLLADASchema"
      : "https://www.khronos.org/collada/";
    var dae = '<?xml version="1.0" encoding="UTF-8" standalone="no" ?>' +
    ...
    dae += `<library_materials>${
      libraryMaterials.join("")
    }</library_materials>`;

    dae += `<library_geometries>${
      libraryGeometries.join("")
    }</library_geometries>`;

    dae +=
      `<library_visual_scenes><visual_scene id="Scene" name="scene">${libraryVisualScenes}</visual_scene></library_visual_scenes>`;
