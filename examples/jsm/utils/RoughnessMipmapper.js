...
RoughnessMipmapper.prototype = {
  ...
  generateMipmaps: function (material) {
    ...
    if (
      roughnessMap === null || normalMap === null ||
    ...
    if (
      _tempTarget === null || _tempTarget.width !== width ||
      _tempTarget.height !== height
    ) {
      if (_tempTarget !== null) _tempTarget.dispose();

      _tempTarget = new WebGLRenderTarget(
        width,
        height,
        { depthBuffer: false },
      );
    ...
    if (
      width !== roughnessMap.image.width || height !== roughnessMap.image.height
    ) {
      var params = {
    ...
    for (
      var mip = 0; width >= 1 && height >= 1; ++mip, width /= 2, height /= 2
    ) {
