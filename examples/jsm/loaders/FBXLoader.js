...
var FBXLoader = (function () {
  ...
  FBXLoader.prototype = Object.assign(Object.create(Loader.prototype), {
    ...
    parse: function (FBXBuffer, path) {
      ...
      var textureLoader = new TextureLoader(this.manager).setPath(
  ...
  FBXTreeParser.prototype = {
      ...
      if ("Video" in fbxTree.Objects) {
        ...
        for (var nodeID in videoNodes) {
          ...
          if ("Content" in videoNode) {
            ...
            var base64Content = (typeof videoNode.Content === "string") &&
              (videoNode.Content !== "");
    ...
    loadTexture: function (textureNode, images) {
      ...
      if (
        ...
        if (
          fileName.indexOf("blob:") === 0 || fileName.indexOf("data:") === 0
        ) {
    ...
    parseParameters: function (materialNode, textureMap, ID) {
      ...
      if (materialNode.Diffuse) {
        ...
        materialNode.DiffuseColor && materialNode.DiffuseColor.type === "Color"
      ) {
    ...
    parseMorphTargets: function (relationships, deformerNodes) {
      ...
      for (var i = 0; i < relationships.children.length; i++) {
        ...
        rawMorphTarget.geoID =
          connections.get(parseInt(child.ID)).children.filter(function (child) {
    ...
    createAmbientLight: function () {
      if (
        "GlobalSettings" in fbxTree && "AmbientColor" in fbxTree.GlobalSettings
      ) {
      ...
      sceneGraph.traverse(function (node) {
        if (node.isMesh) {
          if (Array.isArray(node.material)) {
            ...
            node.material.uuid === matUuid && node.uuid !== uuid
          ) {
            sharedMat = true;
          }
        }
  ...
  GeometryParser.prototype = {
    ...
    parseGeoNode: function (geoNode, skeleton) {
      ...
      geoInfo.vertexPositions = (geoNode.Vertices !== undefined)
        ? geoNode.Vertices.a
        : [];
      geoInfo.vertexIndices = (geoNode.PolygonVertexIndex !== undefined)
    ...
    genMorphGeometry: function (
      ...
      var vertexIndices = (parentGeoNode.PolygonVertexIndex !== undefined)
        ? parentGeoNode.PolygonVertexIndex.a
        : [];

      var morphPositionsSparse = (morphGeoNode.Vertices !== undefined)
        ? morphGeoNode.Vertices.a
        : [];
      var indices = (morphGeoNode.Indexes !== undefined)
  ...
  AnimationParser.prototype = {
            ...
            if (curveNodesMap.has(child.ID)) {
              ...
              if (
                ...
                if (layerCurveNodes[i] === undefined) {
                  var modelID =
                    connections.get(child.ID).parents.filter(function (parent) {
    ...
    generateTracks: function (rawTracks) {
      ...
      initialRotation = new Euler().setFromQuaternion(
        initialRotation,
      ...
      if (
        rawTracks.T !== undefined && Object.keys(rawTracks.T.curves).length > 0
      ) {
      ...
      if (
        rawTracks.R !== undefined && Object.keys(rawTracks.R.curves).length > 0
      ) {
      ...
      if (
        rawTracks.S !== undefined && Object.keys(rawTracks.S.curves).length > 0
      ) {
    ...
    generateMorphTrack: function (rawTracks) {
      ...
      var morphNum =
        sceneGraph.getObjectByName(rawTracks.modelName)
  ...
  BinaryParser.prototype = {
    ...
    parseNode: function (reader, version) {
      ...
      var endOffset = (version >= 7500)
        ? reader.getUint64()
        : reader.getUint32();
      var numProperties = (version >= 7500)
        ? reader.getUint64()
        : reader.getUint32();
    ...
    parseSubNode: function (name, node, subNode) {
      ...
      if (subNode.singleProperty === true) {
        ...
        if (
          innerPropType1 === "Color" || innerPropType1 === "ColorRGB" ||
  ...
  function generateTransform(transformData) {
    ...
    var inheritType = (transformData.inheritType)
    ...
    if (inheritType === 0) {
      ...
      var lParentGSM_noLocal = new Matrix4().multiply(lParentGSM).multiply(
        lParentLSM_inv,
      );

      lGlobalRS.copy(lParentGRM).multiply(lLRM).multiply(lParentGSM_noLocal)
        .multiply(lScalingM);
    }
    ...
    lTransform.copy(lTranslationM).multiply(lRotationOffsetM).multiply(
      lRotationPivotM,
    ).multiply(lPreRotationM).multiply(lRotationM).multiply(lPostRotationM)
      .multiply(lRotationPivotM_inv).multiply(lScalingOffsetM).multiply(
        lScalingPivotM,
      ).multiply(lScalingM).multiply(lScalingPivotM_inv);

    var lLocalTWithAllPivotAndOffsetInfo = new Matrix4().copyPosition(
      lTransform,
    );

    var lGlobalTranslation = new Matrix4().copy(lParentGX).multiply(
