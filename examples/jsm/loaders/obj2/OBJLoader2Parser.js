...
OBJLoader2Parser.prototype = {
  ...
  _checkSubGroup: function () {
    let index = this.rawMesh.activeMtlName + "|" +
      this.rawMesh.smoothingGroup.normalized;
  ...
  _buildFace: function (faceIndexV, faceIndexU, faceIndexN) {
    ...
    if (this.useIndices) {
      ...
      let mappingName = faceIndexV + (faceIndexU ? "_" + faceIndexU : "_n") +
  ...
  _processCompletedMesh: function () {
    ...
    if (haveMesh) {
      ...
      let progressBytesPercent = this.globalCounts.currentByte /
  ...
  _buildMesh: function (result) {
    ...
    let indexUA = (result.absoluteIndexCount > 0)
      ? new Uint32Array(result.absoluteIndexCount)
      : null;
    let colorFA = (result.absoluteColorCount > 0)
      ? new Float32Array(result.absoluteColorCount)
      : null;
    let normalFA = (result.absoluteNormalCount > 0)
      ? new Float32Array(result.absoluteNormalCount)
      : null;
    let uvFA = (result.absoluteUvCount > 0)
