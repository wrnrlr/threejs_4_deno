/// <reference types="./OBJLoader2Parser.d.ts" />
/**
...
OBJLoader2Parser.prototype = {
  ...
  setCallbackOnAssetAvailable: function (onAssetAvailable) {
    if (
      onAssetAvailable !== null && onAssetAvailable !== undefined &&
  ...
  setCallbackOnProgress: function (onProgress) {
    if (
      onProgress !== null && onProgress !== undefined &&
  ...
  setCallbackOnError: function (onError) {
    if (
      onError !== null && onError !== undefined && onError instanceof Function
    ) {
  ...
  _configure: function () {
    ...
    if (this.logging.enabled) {
      ...
      printedConfig += "\n\tcallbacks.onProgress: " +
        this.callbacks.onProgress.name;
  ...
  _pushSmoothingGroup: function (smoothingGroup) {
    ...
    this.rawMesh.smoothingGroup.normalized =
      this.rawMesh.smoothingGroup.splitMaterials
        ? smoothingGroupInt
        : (smoothingGroupInt === 0)
        ? 0
        : 1;
  ...
  _checkSubGroup: function () {
    const index = this.rawMesh.activeMtlName + "|" +
      this.rawMesh.smoothingGroup.normalized;
  ...
  _buildFace: function (faceIndexV, faceIndexU, faceIndexN) {
    ...
    if (this.useIndices) {
      ...
      const mappingName = faceIndexV + (faceIndexU ? "_" + faceIndexU : "_n") +
  ...
  _processCompletedMesh: function () {
    ...
    if (haveMesh) {
      if (
        this.colors.length > 0 && this.colors.length !== this.vertices.length
      ) {
      ...
      const progressBytesPercent = this.globalCounts.currentByte /
        this.globalCounts.totalBytes;
      this._onProgress(
        "Completed [o: " + this.rawMesh.objectName + " g:" +
          this.rawMesh.groupName + "" +
  ...
  _buildMesh: function (result) {
    ...
    const indexUA = (result.absoluteIndexCount > 0)
      ? new Uint32Array(result.absoluteIndexCount)
      : null;
    const colorFA = (result.absoluteColorCount > 0)
      ? new Float32Array(result.absoluteColorCount)
      : null;
    const normalFA = (result.absoluteNormalCount > 0)
      ? new Float32Array(result.absoluteNormalCount)
      : null;
    const uvFA = (result.absoluteUvCount > 0)
    ...
    for (const oodIndex in meshOutputGroups) {
      ...
      if (this.rawMesh.faceType < 4) {
        materialName = materialNameOrg + (haveVertexColors
          ? "_vertexColor"
          : "") +
          (meshOutputGroup.smoothingGroup === 0 ? "_flat" : "");
      } else {
        materialName = this.rawMesh.faceType === 6
      ...
      if (
        ...
        if (this.logging.enabled) {
          console.info(
            'object_group "' + meshOutputGroup.objectName + "_" +
    ...
    this.callbacks.onAssetAvailable(
      {
        ...
        type: "mesh",
        progress: {
          numericalValue: this.globalCounts.currentByte /
            this.globalCounts.totalBytes,
        },
        params: {
        ...
        geometryType: this.rawMesh.faceType < 4
          ? 0
