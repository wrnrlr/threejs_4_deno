/// <reference types="./WorkerExecutionSupport.d.ts" />
/// <reference lib="dom" />
/**
...
WorkerExecutionSupport.prototype = {
    ...
    if (!workerAvailable) {
      try {
        const worker = new Worker(
          codeBuilderInstructions.jsmWorkerUrl.href,
          { type: "module" },
        );
  ...
  _buildWorkerCheckPreconditions: function (requireJsmWorker, timeLabel) {
    ...
    if (this.isWorkerLoaded(requireJsmWorker)) {
      ...
      if (this.logging.enabled) {
        console.info(
          "WorkerExecutionSupport: Building " + (requireJsmWorker
            ? "jsm"
  ...
  _receiveWorkerMessage: function (event) {
    ...
    switch (payload.cmd) {
      ...
      case "completeOverall":
        ...
        if (this.worker.terminateWorkerOnLoad) {
          if (this.worker.logging.enabled) {
            console.info(
              "WorkerSupport [" + workerRunnerName +
      ...
      case "error":
        console.error(
          "WorkerSupport [" + workerRunnerName + "]: Reported error: " +
            payload.msg,
        );
        this.worker.queuedMessage = null;
        this.worker.started = false;
      ...
      default:
        console.error(
          "WorkerSupport [" + workerRunnerName +
            "]: Received unknown command: " + payload.cmd,
        );
        break;
    }
  },
  ...
  _verifyWorkerIsAvailable: function (payload, transferables) {
    ...
    if (this.worker.queuedMessage !== null) {
      ...
      this.worker.queuedMessage = {
        payload: payload,
        transferables: (transferables === undefined || transferables === null)
          ? []
          : transferables,
      };
