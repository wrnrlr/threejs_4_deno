...
class KTX2Loader extends CompressedTextureLoader {
  ...
  load(url, onLoad, onProgress, onError) {
    ...
    Promise.all([bufferPending, this.basisModulePending]).then(
      function ([buffer]) {
  ...
  parse(buffer, onLoad, onError) {
    var BasisLzEtc1sImageTranscoder =
      this.basisModule.BasisLzEtc1sImageTranscoder;
    ...
    ktx.initMipmaps(transcoder, this.transcoderConfig)
      .then(function () {
        ...
        texture.minFilter = ktx.mipmaps.length === 1
          ? LinearFilter
          : LinearMipmapLinearFilter;
...
class KTX2Container {
  constructor(basisModule, arrayBuffer) {
    ...
    this.dfd.numSamples = (this.dfd.descriptorBlockSize / 4 - sampleStart) /
      sampleWords;
    ...
    var selectorsByteOffset = endpointsByteOffset +
  ...
  async initMipmaps(transcoder, config) {
    ...
    var pvrtcTranscodable = MathUtils.isPowerOfTwo(width) &&
    ...
    for (var level = 0; level < this.header.levelCount; level++) {
      ...
      for (var imageIndex = 0; imageIndex < numImagesInLevel; imageIndex++) {
        ...
        if (texFormat === TextureFormat.UASTC4x4) {
          ...
          imageInfo.alphaByteOffset = imageDesc.alphaSliceByteOffset > 0
        ...
        mipmaps.push(
          { data: levelData, width: levelWidth, height: levelHeight },
        );
