    ...
    parse: function (data, path) {
      ...
      function createTokens() {
        ...
        var HexLiteral = createToken(
          { name: "HexLiteral", pattern: /0[xX][0-9a-fA-F]+/ },
        );
        ...
        var FalseLiteral = createToken(
          { name: "FalseLiteral", pattern: /FALSE/ },
        );
      ...
      function buildBackgroundNode(node) {
        ...
        if (skyColor) {
          ...
          var skyMaterial = new MeshBasicMaterial(
            { fog: false, side: BackSide, depthWrite: false, depthTest: false },
          );
        ...
        for (var i = 0, l = fields.length; i < l; i++) {
          ...
          switch (fieldName) {
            case "image":
              ...
              for (
                var j = 3, k = 0, jl = fieldValues.length; j < jl; j++, k++
              ) {
      ...
      function buildExtrusionNode(node) {
        ...
        for (var i = 0; i < spineCount - 1; i++) {
          for (var j = 0; j < crossSectionCount - 1; j++) {
            ...
            if (
              (j === crossSectionCount - 2) && (crossSectionClosed === true)
            ) {
          ...
          for (var j = 1; j < thresholds.length; j++) {
            ...
            if (topDown === true) {
              ...
              if (position.y <= thresholdA.y && position.y > thresholdB.y) {
                t = Math.abs(thresholdA.y - position.y) /
