/// <reference types="./SVGLoader.d.ts" />
/// <reference lib="dom" />
import {
  BufferGeometry,
  FileLoader,
...
SVGLoader.prototype = Object.assign(Object.create(Loader.prototype), {
  ...
  parse: function (text) {
    ...
    function parseCSSStylesheet(node) {
      if (
        !node.sheet || !node.sheet.cssRules || !node.sheet.cssRules.length
      ) {
        return;
      }
    ...
    function parseArcCommand(
      ...
      var x1p = Math.cos(x_axis_rotation) * dx2 +
        Math.sin(x_axis_rotation) * dy2;
      var y1p = -Math.sin(x_axis_rotation) * dx2 +
      ...
      var cx = Math.cos(x_axis_rotation) * cxp -
        Math.sin(x_axis_rotation) * cyp + (start.x + end.x) / 2;
      var cy = Math.sin(x_axis_rotation) * cxp +
        Math.cos(x_axis_rotation) * cyp + (start.y + end.y) / 2;
      ...
      if (nextPoint !== undefined) {
        ...
        if (dot !== 0) {
          ...
          switch (style.strokeLineJoin) {
            ...
            case "miter-clip":
            default:
              var miterFraction = (strokeWidth2 * style.strokeMiterLimit) /
                miterLength2;
              ...
              if (miterFraction < 1) {
                ...
                if (style.strokeLineJoin !== "miter-clip") {
                  ...
                  if (joinIsOnLeftSide) {
                    tempV2_6.subVectors(outerPoint, currentPointL)
                      .multiplyScalar(miterFraction).add(currentPointL);
                    tempV2_7.subVectors(outerPoint, nextPointL).multiplyScalar(
                      miterFraction,
                    ).add(nextPointL);
                    ...
                    tempV2_6.subVectors(outerPoint, currentPointR)
                      .multiplyScalar(miterFraction).add(currentPointR);
                    tempV2_7.subVectors(outerPoint, nextPointR).multiplyScalar(
                      miterFraction,
                    ).add(nextPointR);
