...
var TransformControls = function (camera, domElement) {
  ...
  this.pointerDown = function (pointer) {
    if (
      this.object === undefined || this.dragging === true ||
      pointer.button !== 0
    ) {
      return;
    }
    ...
    if (this.axis !== null) {
      ...
      if (planeIntersect) {
        ...
        if (this.mode === "scale") {
          ...
          this.axis === "E" || this.axis === "XYZE" || this.axis === "XYZ"
        ) {
          space = "world";
        }
        ...
        if (space === "local" && this.mode === "rotate") {
          ...
          if (this.axis === "X" && snap) {
            this.object.rotation.x = Math.round(this.object.rotation.x / snap) *
              snap;
          }
          if (this.axis === "Y" && snap) {
            this.object.rotation.y = Math.round(this.object.rotation.y / snap) *
              snap;
          }
          if (this.axis === "Z" && snap) {
            this.object.rotation.z = Math.round(this.object.rotation.z / snap) *
              snap;
          }
        }
  ...
  this.pointerMove = function (pointer) {
    ...
    if (
      object === undefined || axis === null || this.dragging === false ||
      pointer.button !== -1
    ) {
      return;
    }
    ...
    if (mode === "translate") {
      ...
      if (axis === "E") {
        ...
        rotationAngle = offset.dot(_tempVector.cross(eye).normalize()) *
          ROTATION_SPEED;
      }
      ...
      if (this.rotationSnap) {
        rotationAngle = Math.round(rotationAngle / this.rotationSnap) *
          this.rotationSnap;
      }
...
var TransformControlsGizmo = function () {
  ...
  this.updateMatrixWorld = function () {
    ...
    for (var i = 0; i < handles.length; i++) {
      ...
      if (handle.tag === "helper") {
        ...
        if (handle.name === "AXIS") {
          ...
          if (this.axis === "X") {
            ...
            if (
              Math.abs(
                alignVector.copy(unitX).applyQuaternion(quaternion).dot(
                  this.eye,
                ),
              ) > 0.9
            ) {
          ...
          if (this.axis === "Y") {
            ...
            if (
              Math.abs(
                alignVector.copy(unitY).applyQuaternion(quaternion).dot(
                  this.eye,
                ),
              ) > 0.9
            ) {
          ...
          if (this.axis === "Z") {
            ...
            if (
              Math.abs(
                alignVector.copy(unitZ).applyQuaternion(quaternion).dot(
                  this.eye,
                ),
              ) > 0.9
            ) {
          ...
          tempVector.set(1e-10, 1e-10, 1e-10).add(this.worldPositionStart).sub(
            this.worldPosition,
          ).multiplyScalar(-1);
      ...
      handle.material._opacity = handle.material._opacity ||
        handle.material.opacity;
      handle.material._color = handle.material._color ||
...
var TransformControlsPlane = function () {
  ...
  this.updateMatrixWorld = function () {
    ...
    unitX.set(1, 0, 0).applyQuaternion(
      space === "local" ? this.worldQuaternion : identityQuaternion,
    );
    unitY.set(0, 1, 0).applyQuaternion(
      space === "local" ? this.worldQuaternion : identityQuaternion,
    );
    unitZ.set(0, 0, 1).applyQuaternion(
      space === "local" ? this.worldQuaternion : identityQuaternion,
    );
